...
def main():
    ...
    for h5file_name in os.listdir(args.in_folder):
        ...
        dataset = f['video/video_name']
        video_name = read_h5file.read_sequence_slice('video/video_name', 0)
        video_name = str(video_name).strip('[]')
        total_frames = dataset.shape[0]-1

        out_folder = args.out_folder
        os.makedirs(out_folder, exist_ok=True)
        video_path = ('/scratch/local/hdd/alyssa/bobsl/bobsl/v1.4/original_data/videos/mp4/'f'{video_name}.mp4')
        # print(video_path)
        ...
        for cnt in range(total_frames):
            ...
            ret, frame = cap.read()
            label = read_h5file.read_sequence_slice('video/label', cnt)
            subtitle_text = read_h5file.read_sequence_slice('video/subtitle', cnt)
            subtitle_text = subtitle_text.decode('utf-8')
            subtitle[frame_number] = subtitle_text
            video_list[cnt]=read_h5file.read_sequence_slice('video/video_name', cnt).item()


            ground_truth[frame_number]=label[0]
            ...
            if ret:
                ...
                if right_exist and left_exist:
                    ...
                    # rightleft = np.array([distance(right_centre, left_centre)])
                ...
                with torch.no_grad():  # No need to compute gradients during inference
                    ...
                    # print(probability)
                    predictions[frame_number]= predicted_class
            cnt+=1

        # newresults = smooth(predictions)
        #results = fingerspelling(predictions, 8,6)
        results = fingerspelling(predictions, 5,4)
        total_tp, total_fn, total_fp, event_matrix, tp, fn, fp = calculate_iou_and_metrics(results, ground_truth,video_list)
        total_tp, total_fn, total_fp, event_matrix, tp_1, fn_1, fp_1 = calculate_iou_and_metrics(predictions, ground_truth,video_list)
        ...
        for ii in range(0,total_frames):
            ...
            if frame_number<17450:
                results[frame_number] = 0
                predictions[frame_number] = 0
            cap.set(cv2.CAP_PROP_POS_FRAMES, frame_number)
            ret, frame = cap.read()
            ...
            if ret:
                ...
                current_subtitle = subtitle[frame_number]
                # ax1.text(10, 30, current_subtitle, fontsize=15, color='white', bbox=dict(facecolor='black', alpha=0.7))

                # --- Bottom: Ground Truth vs Model Results Timeline ---
                # Plot the timeline with a fixed x-axis range
                ax2.plot(range(num_frames), ground_truth, color='blue', lw=2, label="Ground Truth")
                ax2.plot(range(num_frames), results, color='red', lw=2, linestyle='--', label="Smooth Results")

                # Set the x-axis limits to the current window range
                ax2.set_xlim(start_index, end_index)

                # # Annotate the current frame with a moving vertical line
                # ax2.axvline(frame_number, color=line_colour, linestyle='--', lw=1.5, label='Current Frame')

                # ax3.plot(range(num_frames), ground_truth, color='blue', lw=2, label="Ground Truth")
                # ax3.plot(range(num_frames), predictions, color='red', lw=2, linestyle='--', label="Results")

                # # Set the x-axis limits to the current window range
                # ax3.set_xlim(start_index, end_index)

                # Annotate the current frame with a moving vertical line
                #ax3.axvline(frame_number, color=line_colour, linestyle='--', lw=1.5)
                # ax3.axvline(51182, color=line_colour, linestyle='--', lw=1.5)
                # ax3.text(51182, 1.7, str(1), ha='center', va='top', fontsize=10)

                # ax3.axvline(51190, color=line_colour, linestyle='--', lw=1.5)
                # ax3.text(51190, 1.7, str(2), ha='center', va='top', fontsize=10)

                # ax3.axvline(51196, color=line_colour, linestyle='--', lw=1.5)
                # ax3.text(51196, 1.7, str(3), ha='center', va='top', fontsize=10)

                # ax3.axvline(51202, color=line_colour, linestyle='--', lw=1.5)
                # ax3.text(51202, 1.7, str(4), ha='center', va='top', fontsize=10)

                # ax2.axvline(51182, color=line_colour, linestyle='--', lw=1.5)
                # ax2.axvline(51190, color=line_colour, linestyle='--', lw=1.5)
                # ax2.axvline(51196, color=line_colour, linestyle='--', lw=1.5)
                # ax2.axvline(51202, color=line_colour, linestyle='--', lw=1.5)

                # ax3.axvline(13700, color=line_colour, linestyle='--', lw=1.5)
                # ax3.text(13700, 1.7, str(1), ha='center', va='top', fontsize=10)

                # ax3.axvline(13704, color=line_colour, linestyle='--', lw=1.5)
                # ax3.text(13704, 1.7, str(2), ha='center', va='top', fontsize=10)

                # ax3.axvline(13710, color=line_colour, linestyle='--', lw=1.5)
                # ax3.text(13710, 1.7, str(3), ha='center', va='top', fontsize=10)

                # ax3.axvline(13715, color=line_colour, linestyle='--', lw=1.5)
                # ax3.text(13715, 1.7, str(4), ha='center', va='top', fontsize=10)

                # ax2.axvline(13700, color=line_colour, linestyle='--', lw=1.5)
                # ax2.axvline(13704, color=line_colour, linestyle='--', lw=1.5)
                # ax2.axvline(13710, color=line_colour, linestyle='--', lw=1.5)
                # ax2.axvline(13715, color=line_colour, linestyle='--', lw=1.5)

                # ax3.axvline(17521, color=line_colour, linestyle='--', lw=1.5)
                # ax3.text(17521, 1.7, str(1), ha='center', va='top', fontsize=10)

                # ax3.axvline(17544, color=line_colour, linestyle='--', lw=1.5)
                # ax3.text(17544, 1.7, str(2), ha='center', va='top', fontsize=10)

                # ax3.axvline(17595, color=line_colour, linestyle='--', lw=1.5)
                # ax3.text(17595, 1.7, str(3), ha='center', va='top', fontsize=10)

                # ax3.axvline(17649, color=line_colour, linestyle='--', lw=1.5)
                # ax3.text(17649, 1.7, str(4), ha='center', va='top', fontsize=10)

                # ax2.axvline(17521, color=line_colour, linestyle='--', lw=1.5)
                # ax2.axvline(17544, color=line_colour, linestyle='--', lw=1.5)
                # ax2.axvline(17595, color=line_colour, linestyle='--', lw=1.5)
                # ax2.axvline(17649, color=line_colour, linestyle='--', lw=1.5)

                # # Optional: Annotate the current frame number with an arrow
                # ax2.annotate(
                #     f'Frame {frame_number}', 
                #     xy=(frame_number, 1.4),  # Position of the arrow tip
                #     xytext=(frame_number, 1.0),  # Position of the arrow base
                ...
                ax2.set_ylabel("Smooth Predictions vs Ground Truth")
                ax2.set_title(f"Video {video_name}")

                # ax3.set_ylim(0, 1.5)  # Adjust y-axis limits as necessary
                # ax3.set_xlabel("Frame Number")
                # ax3.set_ylabel("Ground Truth vs Frame-wise/")
                metrics_text = f"TP: 3, FN: 2, FP: 1"
                ax2.text(
                0.1, 0.8, metrics_text,  # Place the text at the center bottom of the plot
                fontsize=12, color='black', ha='center', transform=ax2.transAxes,
                bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3')
            )
                
            #     metrics_all_text = f"TP: {truep1}, FN: {falsen1}, FP: {falsep1}"
            #     ax3.text(
            #     0.15, 0.7, metrics_all_text,  # Place+ the text at the center bottom of the plot
            #     fontsize=12, color='black', ha='center', transform=ax3.transAxes,
            #     bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3')
            # )

            output_path = os.path.join(out_folder, f'{frame_number}_all.jpg')
